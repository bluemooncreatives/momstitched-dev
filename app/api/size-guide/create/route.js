...
export async function POST(request) {
    try {
        ...
        const existingSlug = await SizeGuideModel.findOne({ slug }).lean()
        if (existingSlug) {
            return response(false, 400, "Slug already exists.")
        }

        const newSizeGuide = new SizeGuideModel({
            name,
            slug,
            type,
            columns: columnValidation.value,
            rows: rowValidation.value,
            note,
        })

        if (parsedIsActive.value !== undefined) {
            newSizeGuide.isActive = parsedIsActive.value
        }

        await newSizeGuide.save()

        return response(true, 200, "Size guide added successfully.", { _id: newSizeGuide._id })
    } catch (error) {
