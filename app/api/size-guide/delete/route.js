...
export async function PUT(request) {
    try {
        ...
        const data = await SizeGuideModel.find({ _id: { $in: ids } }).lean()
        if (!data.length) {
        ...
        if (deleteType === 'SD') {
            await SizeGuideModel.updateMany({ _id: { $in: ids } }, { $set: { deletedAt: new Date().toISOString() } })
        } else {
            await SizeGuideModel.updateMany({ _id: { $in: ids } }, { $set: { deletedAt: null } })
        }
...
export async function DELETE(request) {
    try {
        ...
        await SizeGuideModel.deleteMany({ _id: { $in: ids } })
